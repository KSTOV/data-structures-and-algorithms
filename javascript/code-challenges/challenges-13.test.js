...
const longestString = (arr) => {
  return arr.reduce( (prev, curr, currIdx) => curr.length > prev.max ? {max: curr.length, maxIdx: currIdx} : prev, {max: null, maxIdx: -1} ).maxIdx;
};
...
const findHappiness = (arr) => {
  let localArray = [];
  arr.map(string => {
    string.includes(':)') ? localArray.push(string):false;
  });
  return localArray;
};
...
const standardizePhoneNumbers = (arr) => {
  let regex = /[() -]/g;
  return arr.map(numStr => numStr.replace(regex, ''));
};
...
const allHappy = (arr) => {
  let regex = /(:\))/;
  return arr.reduce((acc, curr) => acc && regex.test(curr), true);
};
